...
func listBigDataServiceInstances(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	...
	logger.Debug("listBdsBdsInstances", "Compartment", compartment, "OCI_REGION", region)
...
func getBigDataServiceInstance(ctx context.Context, d *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	...
	region := plugin.GetMatrixItem(ctx)[matrixKeyRegion].(string)
	compartment := plugin.GetMatrixItem(ctx)[matrixKeyCompartment].(string)
	logger.Debug("getBigDataServiceInstance", "Compartment", compartment, "OCI_REGION", region)
	if h.Item == nil && !strings.HasPrefix(compartment, "ocid1.tenancy.oc1") {
		return nil, nil
	}

	request := buildGetBdsBdsInstanceFilters(d.KeyColumnQuals, h)

	// Create Session
	session, err := bdsService(ctx, d, region)
	if err != nil {
		logger.Error("getBigDataServiceInstance", "error_BdsService", err)
		return nil, err
	}
	request.RequestMetadata = common.RequestMetadata{
		RetryPolicy: getDefaultRetryPolicy(d.Connection),
...
func buildGetBdsBdsInstanceFilters(equalQuals plugin.KeyColumnEqualsQualMap, h *plugin.HydrateData) bds.GetBdsInstanceRequest {
	request := bds.GetBdsInstanceRequest{}

	if h.Item != nil {
		request.BdsInstanceId = h.Item.(bds.BdsInstanceSummary).Id
	} else {
		request.BdsInstanceId = types.String(equalQuals["id"].GetStringValue())
	}
