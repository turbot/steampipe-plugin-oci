...
func listCoreSecurityLists(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	...
	for pagesLeft {
		...
		for _, securityList := range response.Items {
			d.StreamListItem(ctx, securityListInfo{securityList, region})
		}
...
func getCoreSecurityLists(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	plugin.Logger(ctx).Trace("getCoreSecurityLists")
	logger := plugin.Logger(ctx)
	region := plugin.GetMatrixItem(ctx)[matrixKeyRegion].(string)
	compartment := plugin.GetMatrixItem(ctx)[matrixKeyCompartment].(string)
	logger.Debug("oci.getCoreSecurityLists", "Compartment", compartment, "OCI_REGION", region)
	...
	return securityListInfo{response.SecurityList, region}, nil
}

//// TRANSFORM FUNCTION

func securityListTags(_ context.Context, d *transform.TransformData) (interface{}, error) {
	securityList := d.HydrateItem.(securityListInfo).SecurityList
