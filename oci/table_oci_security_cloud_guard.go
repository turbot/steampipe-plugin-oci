...
func tableSecurityCloudGuard(_ context.Context) *plugin.Table {
	return &plugin.Table{
		Name:        "oci_security_cloud_guard",
		Description: "OCI Security Cloud Guard",
		Get: &plugin.GetConfig{
			KeyColumns: plugin.AnyColumn([]string{"id"}),
			Hydrate:    getSecurityCloudGuard,
		},
		List: &plugin.ListConfig{
			Hydrate: listSecurityCloudGuards,
		},
...
func listSecurityCloudGuards(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	logger := plugin.Logger(ctx)
	region := plugin.GetMatrixItem(ctx)[matrixKeyRegion].(string)
	compartment := plugin.GetMatrixItem(ctx)[matrixKeyCompartment].(string)
	logger.Debug("oci.listSecurityCloudGuards", "Compartment", compartment, "OCI_REGION", region)
	...
	pagesLeft := true
	for pagesLeft {
		response, err := session.CloudGuardClient.ListProblems(ctx, request)
		if err != nil {
			return nil, err
		}
...
func getSecurityCloudGuard(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	logger := plugin.Logger(ctx)
	region := plugin.GetMatrixItem(ctx)[matrixKeyRegion].(string)
	compartment := plugin.GetMatrixItem(ctx)[matrixKeyCompartment].(string)
	logger.Debug("oci.getSecurityCloudGuard", "Compartment", compartment, "OCI_REGION", region)
	...
	request := cloudguard.GetProblemRequest{
		ProblemId: types.String(id),
		RequestMetadata: oci_common.RequestMetadata{
			RetryPolicy: getDefaultRetryPolicy(),
		},
	}

	response, err := session.CloudGuardClient.GetProblem(ctx, request)
	if err != nil {
		return nil, err
	}

	return response.Problem, nil
}
...
// 2. Defined Tags
// 3. Free-form tags
func cloudGuardTags(_ context.Context, d *transform.TransformData) (interface{}, error) {
